...
public class UsuarioController {
  ...
  @PutMapping("/usuarios/perfil/{id}")
  public ResponseEntity<?> updatePerfil(@PathVariable("id") Integer id, @RequestBody Usuario u) {
    Optional<Usuario> data = repository.findById(id);
    if (data.isPresent()) {
      Usuario usuario = data.get();
      usuario.setNombre(u.getNombre());
      ...
      usuario.setUsuario(u.getUsuario());
      repository.save(usuario);
      return ResponseController.success("Perfil Actualizado Correctamente", usuario);
    } else {
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
  }
  ...
  public ResponseEntity<?> updateClave(@PathVariable("id") Integer id, @RequestBody Usuario u) {
    ...
    if (data.isPresent()) {
      ...
      usuario.setClave(passwordEncoder.encode(u.getClave())); 
      repository.save(usuario);
      return ResponseController.success("Clave Actualizada Correctamente", usuario);
    } else {
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    }
  }
